...
<div style="float:left; width:100%;">
 ...
 <div id="container" style="width:20%;height:20%;"> 
		<label>Tanggal Jual</label>
			
		<form id="start" action="/pos-kios/index.php/indexController/getDateFilter" method="POST" >
		<input type="date" name="date" class="form-control">
	<input type="submit" name="submit" value="Apply Filter" />
	</form>
	
	</div>
	<div id="container" style="width:20%;height:50%;">
	<label>Tambah Data</label>
	<form id="start" action="/pos-kios/index.php/indexController/tambahPenjualan" method="POST" >
	...
	<?php 
		...
		foreach ($posts as $post){
			$a=(($post->harga_jual-$post->harga_beli)*$post->qty);
			$b=$a+$b;
		}
		echo "Rp";
	...
	<form id="start" action="/pos-kios/index.php/indexController/printFilter" method="POST">
	<input type="date" name="date" class="form-control">
    ...
    <table>
	 ...
	 <td><strong>ID</strong></td>
	  ...
	  <td><strong>Tanggal Jual</strong></td>
     ...
     <tr>
	 <td><?php echo $post->id_trans_penjualan;$no++;?></td>
         <td><?php echo $post->nama_barang;?></td>
         <td><?php echo $post->qty;?></td>
         <td><?php echo number_format($post->harga_beli);?></td>
         <td><?php echo number_format($post->harga_jual);?></td>
		 
         <td><?php echo $post->tanggal_jual;?></td>
		 <td><?php echo '<form id="start" action="/pos-kios/index.php/indexController/editPenjualan" method="POST" >';?>
         <?php echo '<input type="submit" name="';
		 echo $post->id_trans_penjualan;
		 echo '" value="Edit" />';?>
		 <?php echo '</form>';?>
		 <?php echo '<form id="start" action="/pos-kios/index.php/indexController/deletePenjualan" method="POST" >';?>
         <?php echo '<input type="submit" name="';
		 echo $post->id_trans_penjualan;
		 echo '" value="Delete" />';?>
         <?php echo '</form>';?></td>
